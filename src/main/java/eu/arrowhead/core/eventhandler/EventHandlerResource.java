...
public class EventHandlerResource {
  ...
  public Response publishEvent(PublishEvent eventPublished, @Context ContainerRequestContext requestContext) {
    ...
    if (ArrowheadMain.PUBLISH_EVENTS_DELAY > 0 && eventPublished.getEvent().getTimestamp()
                                                                .isBefore(LocalDateTime.now().minusMinutes(ArrowheadMain.PUBLISH_EVENTS_DELAY))) {
      throw new BadPayloadException(
          "This event is too old to publish. Maximum allowed delay before publishing the event: " + ArrowheadMain.PUBLISH_EVENTS_DELAY);
    }
