...
public class GatekeeperResource {
	...
	public Response GSDRequest(GSDRequestForm requestForm) {
		if (!requestForm.isValid()) {
			...
			throw new BadPayloadException(
					"init_gsd received bad payload: requestedService is missing or it is not valid.");
		}
	...
	public Response GSDPoll(GSDPoll gsdPoll) {
		if (!gsdPoll.isValid()) {
			...
			throw new BadPayloadException(
					"gsd_poll received bad payload: requestedService/requesterCloud is missing or it is not valid.");
		}
		...
		InterCloudAuthRequest authRequest = new InterCloudAuthRequest(gsdPoll.getRequesterCloud(),
				gsdPoll.getRequestedService(), false);
		...
		if (!authResponse.readEntity(InterCloudAuthResponse.class).isAuthorized()) {
			...
			ErrorMessage errorMessage = new ErrorMessage(
					"Requester Cloud is UNAUTHORIZED to consume this service, GSD poll failed.", 401,
		...
		// If it is authorized, poll the Service Registry for the requested Service
			...
			if (!result.isValid()) {
				...
				ErrorMessage errorMessage = new ErrorMessage(
						"Service not found in the Service Registry, GSD poll failed.", 404,
	...
	public Response ICNProposal(ICNProposal icnProposal) {
		...
		InterCloudAuthRequest authRequest = new InterCloudAuthRequest(icnProposal.getRequesterCloud(),
				icnProposal.getRequestedService(), false);
		String authUri = Utility.getAuthorizationUri();
		authUri = UriBuilder.fromPath(authUri).path("intercloud").toString();
		Response authResponse = Utility.sendRequest(authUri, "PUT", authRequest);
		...
		if (!authResponse.readEntity(InterCloudAuthResponse.class).isAuthorized()) {
			...
			ErrorMessage errorMessage = new ErrorMessage(
					"Requester Cloud is UNAUTHORIZED to consume this service, ICNProposal failed.", 401,
					AuthenticationException.class.toString());
			return Response.status(Status.UNAUTHORIZED).entity(errorMessage).build();
		}
