				...
				if (i.isUp() && !i.isLoopback()) {
					for (InterfaceAddress ifaddr : i.getInterfaceAddresses()) {
						...
						try {
							...
							logger.warn("Bad hostname", ex);
						} catch (UnknownHostException ex) {
							logger.debug("No hostname for address: " + inetAddr);
						}
						try {
							...
							logger.warn("Failed to calculate network address", ex);
						}
					}
				}
			}
		} catch (SocketException ex) {
			logger.warn("Failed to enumerate network interfaces", ex);
		}
		return results;
	}
