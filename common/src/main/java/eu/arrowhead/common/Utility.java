...
public final class Utility {
	...
	public static <T> Response sendRequest(String uri, String method, T payload) {
		...
		Builder request = client.target(UriBuilder.fromUri(uri).build()).request().header("Content-type",
				"application/json");
		...
		if (!(response.getStatusInfo().getFamily() == Family.SUCCESSFUL)) {
			...
			try {
				...
				throw new RuntimeException(
						"Unknown error occurred at " + uri + ". Check log for possibly more information.");
			}
			if (errorMessage == null) {
				...
				throw new RuntimeException(
						errorMessage.getErrorMessage() + " (This exception was passed from another module)");
			} else {
				log.error("Request returned with " + errorMessage.getExceptionType() + ": "
						+ errorMessage.getErrorMessage());
				throw new RuntimeException(errorMessage.getErrorMessage() + " (This " + errorMessage.getExceptionType()
						+ " was passed from another module)");
			}
		}
	...
	public static String getOrchestratorUri() {
		...
		return getUri(orchestrator.getAddress(), orchestrator.getPort(), orchestrator.getServiceURI(),
				orchestrator.getIsSecure());
	}
	...
	public static List<String> getNeighborCloudURIs() {
		...
		for (NeighborCloud cloud : cloudList) {
			uriList.add(getUri(cloud.getCloud().getAddress(), cloud.getCloud().getPort(),
					cloud.getCloud().getGatekeeperServiceURI(), false));
		}
