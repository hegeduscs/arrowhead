...
public class ApiResource {
	...
	public List<ArrowheadSystem> addSystems(List<ArrowheadSystem> systemList) {
		...
		for (ArrowheadSystem system : systemList) {
			ArrowheadSystem retrievedSystem = dm.getArrowheadSystem(system.getSystemGroup(), system.getSystemName());
	...
	public List<ArrowheadCloud> addClouds(List<ArrowheadCloud> cloudList) {
		...
		for (ArrowheadCloud cloud : cloudList) {
			ArrowheadCloud retrievedCloud = dm.getArrowheadCloud(cloud.getOperator(), cloud.getCloudName());
	 ...
	 * Returns 200 if the delete is succesful, 204 (no content) if the cloud was
