...
class UnicastDnsSDRegistrator implements DnsSDRegistrator {
	...
	private static final Logger logger = Logger
			.getLogger(UnicastDnsSDBrowser.class.getName());

	private static final Name DNSUPDATE_UDP = Name
			.fromConstantString("_dns-update._udp");
	private static final Name SERVICES_DNSSD_UDP = Name
			.fromConstantString("_services._dns-sd._udp");
	 ...
	 *             if the DNS server name for the domain failed to resolve.
	 */
	UnicastDnsSDRegistrator(Name registrationDomain)
			throws UnknownHostException {
		try {
			this.registrationDomain = registrationDomain;
			this.resolver = findUpdateResolver(registrationDomain);
			this.servicesName = Name.concatenate(SERVICES_DNSSD_UDP,
					registrationDomain);
			logger.log(Level.INFO, "Created DNS-SD Registrator for domain {0}",
					registrationDomain);
		} catch (NameTooLongException e) {
			throw new IllegalArgumentException("Domain name too long: "
					+ registrationDomain, e);
		}
	}
	 ...
	 * 
	 * @param domain
	 *            the domain for which updates will be generated.
	 * @return a Resolver configured with the DNS server that handles zone for
	 *         that domain.
	 * @throws UnknownHostException
	 *             if the DNS server name for the domain failed to resolve.
	 */
	private Resolver findUpdateResolver(Name domain)
			throws UnknownHostException {
		SimpleResolver simpleResolver = null;
		try {
			Lookup lookup = new Lookup(Name.concatenate(DNSUPDATE_UDP, domain),
					Type.SRV);
			...
			if (records != null) {
				for (Record record : records) {
					if (record instanceof SRVRecord) {
						...
						InetAddress addr = Address.getByName(srv.getTarget()
								.toString());
						InetSocketAddress socaddr = new InetSocketAddress(addr,
								srv.getPort());
						logger.log(Level.INFO,
								"Using DNS server {0} to perform updates.",
								socaddr);
	...
	public boolean registerService(ServiceData serviceData)
			throws DnsSDException {
		try {
			ServiceName serviceName = serviceData.getName();
			Name dnsName = serviceName.toDnsName();
			Name typeName = new Name(serviceName.getType().toDnsString(),
					registrationDomain);
			List<Name> subtypes = new ArrayList<Name>(serviceName.getType()
					.getSubtypes().size());
			for (String subtype : serviceName.getType()
					.toDnsStringsWithSubtype()) {
				subtypes.add(new Name(subtype, registrationDomain));
			}
			Name target = new Name(serviceData.getHost());
			List<String> strings = new ArrayList<String>();
			for (Map.Entry<String, String> entry : serviceData.getProperties()
					.entrySet()) {
			...
			Update update = new Update(registrationDomain); // XXX Should really
															// be the zone (SOA)
															// for the RRs we
															// are about to add
			update.absent(dnsName);
			update.add(new PTRRecord(servicesName, DClass.IN, timeToLive,
					typeName));
			update.add(new PTRRecord(typeName, DClass.IN, timeToLive, dnsName));
			for (Name subtype : subtypes) {
				update.add(new PTRRecord(subtype, DClass.IN, timeToLive,
						dnsName));
			}
			update.add(new SRVRecord(dnsName, DClass.IN, timeToLive, 0, 0,
					serviceData.getPort(), target));
			...
			case Rcode.NOERROR:
				flushCache(update);
				return true;
			case Rcode.YXDOMAIN: // Prerequisite failed, the service already
									// exists.
				return false;
			default:
				throw new DnsSDException("Server returned error code: "
	...
	public boolean unregisterService(ServiceName serviceName)
			throws DnsSDException {
		try {
			Name dnsName = serviceName.toDnsName();
			Name typeName = new Name(serviceName.getType().toDnsString(),
			...
			update.delete(new PTRRecord(typeName, DClass.IN, timeToLive,
					dnsName));
			for (Name subtype : subtypes) {
				update.delete(new PTRRecord(subtype, DClass.IN, timeToLive,
						dnsName));
			}
			...
			case Rcode.NOERROR:
				flushCache(update);
				break;
			case Rcode.NXDOMAIN: // Prerequisite failed, the service doesn't
									// exist.
				return false;
			default:
				throw new DnsSDException("Server returned error code: "
			...
			update = new Update(registrationDomain); // XXX Should really be the
														// zone (SOA) for the
														// RRs we are about to
														// remove
			update.absent(typeName);
			update.delete(new PTRRecord(servicesName, DClass.IN, timeToLive,
					typeName));
			response = resolver.send(update);
			switch (response.getRcode()) {
			case Rcode.NOERROR:
				flushCache(update);
				logger.log(Level.FINE, "Removed service type record {0}",
						typeName);
				break;
			case Rcode.YXDOMAIN: // Prerequisite failed, service instances
									// exists
				logger.log(
						Level.FINE,
						"Did not remove service type record {0}, instances left.",
						typeName);
				break;
			default:
				logger.log(
						Level.WARNING,
						"Failed to remove service type {0}, server returned status {1}",
						new Object[] { typeName,
								Rcode.string(response.getRcode()) });
			}
			return true;
	...
	private static void flushCache(Update update) {
		...
		for (Record rec : records) {
			logger.log(Level.FINE, "Flush name {0} due to update: {1}",
					new Object[] { rec.getName(), rec });
